...
public class AAAOpenPositionAtSpecificTime : QCAlgorithm, IRegressionAlgorithmDefinition
{
    ...
    public override void OnData(Slice slice)
    {
        if (slice.First().Value is AAAMinute daily)
        {
            ...
            if (currentBar.Time.Hour == 20 && currentBar.Time.Minute == 1) {
                var marketOrder = MarketOrder(symbol, 1);
                var limitOrder = LimitOrder(symbol, -1, 3117);
                Console.WriteLine(marketOrder + " " + limitOrder);
            }
        }
    }
